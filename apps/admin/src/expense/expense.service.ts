...
export class ExpenseService {
  ...
  // Expenses feed net-revenue dashboards — negative values would corrupt them.
  private assertValidAmount(amount: number): void {
    if (typeof amount !== 'number' || !Number.isFinite(amount) || amount < 0) {
      throw new BadRequestException('المبلغ يجب أن يكون رقماً غير سالب');
    }
  }
  async getAll() {
  ...
  async create(data: { amount: number; reason: string }) {
    ...
    this.wsGateway.emitToAdmin(WS_EVENTS.FINANCIAL_UPDATED, {
  ...
  async getTotal() {
    const r = await this.prisma.expense.aggregate({ _sum: { amount: true } });
    return Number(r._sum.amount ?? 0);
  }
  private async findOne(id: string) {
    const e = await this.prisma.expense.findUnique({ where: { id } });
    if (!e) throw new NotFoundException('المصروف غير موجود');
    return e;
  }
}
