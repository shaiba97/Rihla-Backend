...
const awardReceiptsDir = path.join(
  __dirname,
  '../../../uploads/award-receipts',
);
mkdir(awardReceiptsDir);

const receiptUpload = (dir: string, prefix: string) =>
...
export class AwardController {
  ...
  @Get('packs') getPacks() {
    return this.svc.getPacks();
  }
  @Post('packs') createPack(@Body() body: any) {
    return this.svc.createPack(body);
  }
  @Patch('packs/:id') updatePack(@Param('id') id: string, @Body() body: any) {
    return this.svc.updatePack(id, body);
  }
  @Delete('packs/:id') removePack(@Param('id') id: string) {
    return this.svc.removePack(id);
  }
  @Get('user/:userId') getUserAwards(@Param('userId') userId: string) {
    return this.svc.getUserAwards(userId);
  }
  ...
  approve(@Param('id') id: string, @UploadedFile() file: Express.Multer.File) {
    return this.svc.approve(
      id,
      captureReceipt(awardReceiptsDir, 'award', file),
    );
  }
  ...
  getWithdrawalHistory() {
    return this.svc.getWithdrawalHistory();
  }
  ...
  approveWithdrawal(
    ...
    return this.svc.approveWithdrawal(
      id,
      captureReceipt(withdrawalsDir, 'withdraw', file),
    );
  }

  @Post('withdrawals/:id/reject')
  rejectWithdrawal(@Param('id') id: string) {
    return this.svc.rejectWithdrawal(id);
  }
}
