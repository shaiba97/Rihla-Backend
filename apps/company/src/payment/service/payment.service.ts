...
export class PaymentService {
  ...
  async getFinancialSummary(companyId: string) {
    ...
    const totalRevenue = payments.reduce(
      (s: number, p: any) => s + Number(p.companyAmount ?? 0),
      0,
    );
    ...
    const totalBookings = await this.prisma.booking.count({
      where: { Trip: { Bus: { companyId } }, status: 'CONFIRMED' },
    });
    const pendingBookings = await this.prisma.booking.count({
      where: { Trip: { Bus: { companyId } }, status: 'PENDING' },
    });
    ...
    const topTrips = Object.values(tripMap)
    ...
    const totalCompanyIncome = payments.reduce(
      (s: number, p: any) => s + Number(p.companyAmount ?? 0),
      0,
    );
    return {
      totalRevenue,
      totalCommission: 0,
      netEarnings: totalCompanyIncome,
      thisMonthRevenue,
      totalBookings,
      pendingBookings,
      dailyRevenue,
  ...
  async getPerformance(
    ...
    payments.forEach((p: any) => {
      ...
      if (!groups[key])
        groups[key] = { revenue: 0, platformFees: 0, netIncome: 0, count: 0 };
